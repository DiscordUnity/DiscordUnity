...
{
    public class DiscordAudioClient : IDisposable
    {
        ...
        internal DiscordAudioClient(DiscordClient pclient, DiscordChannel pchannel)
        {
            parent = pclient;
            channel = pchannel;
        ...
        public void Dispose()
        {
            ...
            parent.unityInvoker.Enqueue(() => parent.OnAudioClientClosed(client, new DiscordAudioClientArgs() { client = parent, audioClient = this }));
        }
        ...
        private void ProcessMessage(int op, string payload)
        {
            switch (op)
            {
                ...
                case 4:
                    ...
                    udpThread.Start();
                    isOnline = true;
                    parent.unityInvoker.Enqueue(() => parent.OnAudioClientOpened(parent, new DiscordAudioClientArgs() { client = parent, audioClient = this }));
        ...
        private void SendVoice()
        {
            ...
            if (buffer != null)
            {
                ...
                parent.unityInvoker.Enqueue(() => OnVoicePacketSend(this, new DiscordVoiceArgs() { channel = channel, client = parent, packet = packet, sender = parent.user }));
