...
namespace DiscordUnity
{
    public class OpusDecoder : OpusConverter
    {
        ...
        /// <param name="inputOffset">Offset of the frame in input.</param>
        /// <param name="output">Buffer to store the decoded frame.</param>
        public unsafe int DecodeFrameFloat(byte[] input, int inputOffset, int inputCount, float[] output)
        {
            int result = 0;
            fixed (byte* inPtr = input)
                result = UnsafeNativeMethods.DecodeFloat(_ptr, inPtr + inputOffset, inputCount, output, SamplesPerFrame, 0);
