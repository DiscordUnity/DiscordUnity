...
namespace DiscordUnity
{
    internal class OpusEncoder : OpusConverter
    {
        ...
        {
            if (bitrate != null && (bitrate < 1 || bitrate > 320))
        ...
        /// <param name="output">Buffer to store the encoded frame.</param>
        /// <returns>Length of the frame contained in outputBuffer.</returns>
        public unsafe int EncodeFrameFloat(float[] input, int inputOffset, byte[] output)
        {
            int result = 0;
            fixed (float* inPtr = input)
                result = UnsafeNativeMethods.EncodeFloat(_ptr, inPtr + inputOffset, SamplesPerFrame, output, output.Length);
