...
namespace DiscordUnity
{
    ...
    public abstract class OpusConverter : IDisposable
    {
        ...
        protected unsafe static class UnsafeNativeMethods
        {
            ...
            public static extern int EncodeFloat(IntPtr st, float* pcm, int frame_size, byte[] data, int max_data_bytes);
            [DllImport("opus", EntryPoint = "opus_decode_float", CallingConvention = CallingConvention.Cdecl)]
            public static extern int DecodeFloat(IntPtr st, byte* data, int len, float[] pcm, int frame_size, int decode_fec);
        }
